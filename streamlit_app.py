...
import streamlit as st
from agent_layer import handle_input_ui, render_chat_history
from langchain_core.messages import HumanMessage, SystemMessage, AIMessage
import time
...
if "initialized" not in st.session_state:
    ...
    st.markdown(
        ...
        💡 *Tip: The more lyrics you share, the better my suggestions get.* </div>
        """,
        unsafe_allow_html=True
    )

    st.session_state.initialized = True
    # Initializing chat history within the initialization block
    if "messages" not in st.session_state:
        st.session_state.messages = []
...
if prompt:
    ...
    with st.chat_message('assistant'):
        ...
        # Pass the session-specific chat history to the handler function
        for chunk in handle_input_ui(prompt, st.session_state.messages):
            response += chunk
            placeholder.markdown(response + "▌")
        placeholder.markdown(response)
